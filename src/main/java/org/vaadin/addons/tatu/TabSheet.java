...
public class TabSheet extends Component implements HasSize, HasTheme {
	...
	public enum TabSheetVariant {
		LUMO_ICON_ON_TOP("icon-on-top"), LUMO_CENTERED("centered"), LUMO_SMALL("small"), LUMO_MINIMAL("minimal"),
		LUMO_HIDE_SCROLL_BUTTONS("hide-scroll-buttons"), LUMO_EQUAL_WIDTH_TABS("equal-width-tabs"),
		BORDERED("bordered"), MATERIAL_FIXED("fixed");
	 ...
	 * <p>
	 * Note: If tab-sheet and component is defined in template, the components are
	 * not available.
	 * 
	...
	public Optional<Component> getComponent(String tab) {
		...
		return getChildren().filter(comp -> comp.getElement().getAttribute("slot").equals(tab)).findFirst();
	}
	 ...
	 * 
	 * @param listener Functional interface, lambda expression of the listener
	 *                 callback function.
	 * @return Listener registration. Use {@link Registration#remove()} to remove
	 *         the listener.
	 */
	public Registration addTabChangedListener(ComponentEventListener<TabChangedEvent<TabSheet>> listener) {
		return addListener(TabChangedEvent.class, (ComponentEventListener) listener);
	}
	...
	/**
	 * Set selected tab using index. This will fire TabChangeEvent. Sheet attached
	 * to the tab will be shown.
	 * 
	 * @param index Index of the tab, base 0.
	 */
	...
	public void addThemeVariants(TabSheetVariant... variants) {
		getThemeNames().addAll(Stream.of(variants).map(TabSheetVariant::getVariantName).collect(Collectors.toList()));
	}
	...
	public void removeThemeVariants(TabSheetVariant... variants) {
		getThemeNames().removeAll(Stream.of(variants).map(TabSheetVariant::getVariantName).collect(Collectors.toList()));
	}
